...
- The ⋮ button in the top-right of the header opens the game menu (`GameMenu`
  component); the only option during an active game is "Quit Game".
...
## Screen 11: Idle / Pre-Game Screen

Shown when no game is in progress — either on first launch (before any game has
been started) or after the player confirms a quit. Tab bar is visible at the
bottom.
...
│   ┌─────────────────────────────────────────────────────┐  │
│   │                                                     │  │
│   │   "You can't win if you never start, but I've       │  │
│   │    seen plenty of people lose by starting wrong."   │  │
│   │                                                     │  │
│   │                             — Grandma               │  │
│   └─────────────────────────────────────────────────────┘  │
│                                                             │
...
**Notes:**
- Quote is drawn randomly from the `opening` pool on each visit to this screen.
- Tab bar is visible; the player can navigate away without starting a game.
- Pressing "Start New Game" initialises a fresh `GameState`, sets `GameActive`
  to `true`, and transitions to the normal game screen (Screen 1/2).

---

## Screen 12: Quit Game Confirmation Overlay

Shown when the player taps ⋮ → "Quit Game" during an active game. Rendered as
a modal overlay above the game screen.

```
┌─────────────────────────────────────────────────────────────┐
│  SIXZEE                              Turn 14  ●●● 3 rolls  ⋮ │
├─────────────────────────────────────────────────────────────┤
│  (game screen dimmed behind overlay)                        │
│                                                             │
│         ┌───────────────────────────────────────┐          │
│         │                                       │          │
│         │         Quit this game?               │          │
...
**Notes:**
- Quote is drawn randomly from the `quit` pool each time the overlay opens.
- "Keep Playing" dismisses the overlay and returns to the game unchanged.
- "Quit Game" discards the in-progress save, clears all game state, sets
  `GameActive` to `false`, and navigates to the Idle screen (Screen 11).
- Tab bar is **hidden** while this overlay is open.
